...
def build_abc_classes(skus: pd.DataFrame) -> pd.DataFrame:
    ...
    df["velocity_per_day"] = pd.to_numeric(
        df["velocity_per_day"], errors="coerce"
    ).fillna(0.0)
...
def infer_sku_home_locations(
    ...
    out = (
        mode_loc[["sku", "pick_location_id"]]
        .rename(columns={"pick_location_id": "location_id"})
        .copy()
    )
...
def generate_inventory_snapshot(
    ...
    known_loc = (
        home["location_id"].dropna().astype(str)
        if not home.empty
    ...
    if "qty" in ord_df.columns:
        ...
        reserved = (
            ord_df.groupby("sku", as_index=False)["qty"]
            .sum()
            .rename(columns={"qty": "reserved_qty"})
        )
    else:
        reserved = (
            ord_df.groupby("sku", as_index=False)
            .size()
            .rename(columns={"size": "reserved_qty"})
        )
    ...
    # Available
    df["available_qty"] = (
        (df["on_hand_qty"] - df["reserved_qty"]).clip(lower=0).astype(int)
    )
    ...
    df["reorder_point"] = (
        (df["velocity_per_day"] * lead_time_days + safety_stock).round().astype(int)
    )
    ...
    df["reorder_qty"] = (
        (df["reorder_point"] + target_stock - df["on_hand_qty"])
        .clip(lower=0)
        .astype(int)
    )
    ...
    df["_abc_sort"] = pd.Categorical(
        df["abc_class"], categories=["A", "B", "C"], ordered=True
    )
    return (
        df[cols + ["_abc_sort"]]
        .sort_values(
            ["_abc_sort", "velocity_per_day"],
